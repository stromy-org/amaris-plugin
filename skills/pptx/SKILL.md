...
```javascript
const BRAND_DIR = path.resolve(__dirname, '../../companies/amaris/brand');
```
...
When a company directory exists, also check for `profile.json` at `companies/amaris/profile.json`. If present, use company identity fields for presentation slides:
...
When a company directory exists, check for `people.json` at `companies/amaris/people.json`. If present, use it for author metadata in footers, contact slides, and file properties. Filter by `roles` containing `"author"` — if one person has `"default": true`, auto-select them.
